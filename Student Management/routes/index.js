...
{ 
  ...
  console.log(regno);
  var query=`select * from mca where Register_Number="${regno}"`;
 ...
 database.query("SELECT * FROM mca",(err, result)  => {
   ...
   //console.log(date);
         ...
         if(atten[i]=='1'){
           ...
           database.query(`INSERT INTO attendence (regno,date,remark) VALUES ('${regno}','${date}','ABSENT')` ,function (err,result){
             if(err){
             console.log(err);
             }
             else{
