...
app.get("/addstudent",(req,res) => {
  //fetching data from form
  const { Name, Register_Number, Institutional_Email, Personal_Email, Mobile } = req.query
  console.log(Name);
  //Sanitization XSS...
  let qry = "select * from mca where Register_Number=?";
  database.query(qry, [Register_Number],(err,results) => {
      if(err)
      throw err
      else{
...
res.render("add",{ checkmesg:true })
          ...
          database.query(qry2,[Name,Register_Number,Institutional_Email,Personal_Email,Mobile],(err,results) => {
...
app.get("/updatestudent", (req,res) => {
  //fetch data
  const { Name, Institutional_Email, Personal_Email, Mobile, Register_Number  } = req.query;
  let qry = "update  mca set Name=?, Institutional_Email=?, Personal_Email=?, Mobile=? where Register_Number=?";

  database.query(qry,[Name, Institutional_Email, Personal_Email, Mobile, Register_Number], (err, results) => {
      if(err)
         throw err
      else {
          if (results.affectedRows > 0) {
